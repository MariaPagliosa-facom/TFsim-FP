...
void res_station_rob::exec()
{
    while(true)
    {
        ...
        if(!isFlushed)
        {
            ...
            auto op_name = op.substr(0, 4);

            if (op_name == "DADD" || op_name == "FADD")
              res = vj + vk;
            else if (op_name == "DSUB" || op_name == "FSUB")
              res = vj - vk;
            else if (op_name == "DMUL" || op_name == "FMUL")
              res = vj * vk;
            else if (op_name == "DDIV" || op_name == "FDIV")
            {
                if(vk)
                    res = vj/vk;
                else
