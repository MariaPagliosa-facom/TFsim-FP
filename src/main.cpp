...
std::string get_base_path(const char **argv) {
...
int sc_main(int argc, char *argv[])
{
    ...
    {"SLT",1},{"SGT", 1},
    {"FADD", 8},{"FADDI", 8},{"FSUB", 12},{"FMUL", 15},{"FDIV", 25}};
    ...
    sub->append("Tempos de latência", [&](menu::item_proxy &ip)
    {
        ...
        inputbox::text fadd_t("FADD", std::to_string(instruct_time["FADD"]));
        inputbox::text faddi_t("FADDI", std::to_string(instruct_time["FADDI"]));
        inputbox::text fsub_t("FSUB", std::to_string(instruct_time["FSUB"]));
        inputbox::text fmul_t("FMUL", std::to_string(instruct_time["FMUL"]));
        inputbox::text fdiv_t("FDIV", std::to_string(instruct_time["FDIV"]));
        if(ibox.show_modal(dadd_t,daddi_t,dsub_t,dsubi_t,dmul_t,ddiv_t,mem_t))
        {
            ...
            instruct_time["FADD"] = std::stoi(fadd_t.value());
            instruct_time["FADDI"] = std::stoi(faddi_t.value());
            instruct_time["FSUB"] = std::stoi(fsub_t.value());
            instruct_time["FMUL"] = std::stoi(fmul_t.value());
            instruct_time["FDIV"] = std::stoi(fdiv_t.value());
        }
    });
    ...
    bench_sub->append("ex1", [&](menu::item_proxy& ip)
      {
        string path = base_path + "/in/benchmarks/ex1/ex1.txt";
        bench_name = "ex1";
        inFile.open(path);
        if (!add_instructions(inFile, instruction_queue, instruct))
          show_message("Arquivo inválido", "Não foi possível abrir o arquivo!");
    ...
    bench_sub->append("ex2", [&](menu::item_proxy& ip)
      {
        string path = base_path + "/in/benchmarks/ex2/ex2.txt";
        bench_name = "ex2";
        inFile.open(path);
        if (!add_instructions(inFile, instruction_queue, instruct))
          show_message("Arquivo inválido", "Não foi possível abrir o arquivo!");
